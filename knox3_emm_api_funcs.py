...
def deleteDevice(tab):
    requests.post('https://us02.manage.samsungknox.com/emm/oapi/device/deleteDeviceByDeviceId',
                  headers=post_header,
                  data='deviceId={}'.format(tab))


def getDeviceByImei(imei):
    post_data = "imei={}".format(imei)
    device_details_response = requests.post(
    ...
    serial_number = device_details_response_json['serialNumber']
    device_id = device_details_response_json['deviceId']
    device_name = device_details_response_json['mobileId']
    foundTab = Tablet(device_name=device_name, serial_number=serial_number, imei=imei, iccid=iccid,
                      device_id=device_id)
    return foundTab
