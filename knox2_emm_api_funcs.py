...
def getAuthToken():
    ...
    data = 'grant_type=client_credentials&client_id=k2test@itdirector.vitaltech.com&client_secret=testing1!'
...
def deviceRange(initial_number, last_number, user):
    ...
    for x in range(initial_number, last_number + 1):
        tablet_name = "k2{}_Android_{}".format(user, x)
...
def getUserDevices(user_name):
    ...
    json_tablet_list = \
        json.loads(requests.post("https://us02.manage.samsungknox.com/emm/oapi/device/selectDevicesByUser",
                                 headers=post_header, data='userId=k2{}'.format(user_name)).text)[
            'resultValue']
